...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--target_size',
        help='Target size of the image NOTE:(WIDTH, HEIGHT). '
    ...
    parser.add_argument(
        ...
        type=pathlib.Path,
        default='/home/user/dataset/MegaDepth/phoenix/S6/zl548/MegaDepth_v1'
    )
    parser.add_argument(
        '--output_path',
        help='Path to directory where extracted features are stored',
        type=pathlib.Path,
        default='/home/user/dataset/MegaDepth/features'
    )
    parser.add_argument(
        '--extractor_config_path',
        help='Path to the file containing config for feature extractor in .yaml format',
        type=pathlib.Path,
        default='/home/user/code/OpenGlue/config/features/superpoint_magicleap.yaml'
    )
    parser.add_argument(
        '--recompute',
...
def main():
    ...
    process(images_list, feature_extractor_config, output_path, args)
    print('SuperPoint Saved Done')
    


def process(images_list: List[Tuple[str, Union[str, None]]], feature_extractor_config: dict,
    ...
    with torch.inference_mode():
        for i, (image_path, scene) in enumerate(images_list, start=1):
            ...
            if i % 100 == 0:
                logger.info(f'Processed {i}/{len(images_list)} images.')
...
def get_images_list(input_data_path: pathlib.Path, image_formats: List[str]) -> List[Tuple[str, Optional[str]]]:
    ...
    print(f'MegaDepth has {len(scenes)} scenes')
    for scene in scenes:
